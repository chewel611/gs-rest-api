...
public class StylesClient {
     ...
     * @param _default 是否默认
     */
    public void addStyleToLayer(String layerName, String styleName, Boolean _default) {
        if (StrUtil.isBlank(layerName)) {
            throw new IllegalArgumentException("layerName is not empty");
        }

        if (ObjUtil.isNull(styleName)) {
            throw new IllegalArgumentException("styleName is not empty");
        }

        StyleInfoPost style = new StyleInfoPost().name(styleName);
        StyleInfoPostWrapper wrapper = new StyleInfoPostWrapper().style(style);

        api().addStyleToLayer(layerName, wrapper, _default);
    }
}
